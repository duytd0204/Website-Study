/**
 * OcrView - Trích xuất dữ liệu từ ảnh (Use Case 3.12).
 * State (currentType, extractedItems, imageFile) là thuộc tính instance.
 */
class OcrView extends BaseView {
  constructor(container) {
    super(container);
    this.currentType = "schedule";
    this.extractedItems = [];
    this.imageFile = null;
  }
  ...
  async render() {
    this.setHTML(`
      ...
      <div class="card">
        <div class="card-header"><h3 class="card-title">1. Chọn loại ảnh</h3></div>
  ...
  _bindTypeTabs() {
    this.$all(".type-btn").forEach(b => {
      b.onclick = () => {
        this.currentType = b.dataset.type;
        this.$all(".type-btn").forEach(b2 => {
          b2.className = b2.dataset.type === this.currentType ? "btn btn-primary type-btn" : "btn btn-secondary type-btn";
        });
      };
    });
  }
  ...
  _bindUploadZone() {
    ...
    this.$("#browseFile").onclick = (e) => { e.preventDefault(); fileInput.click(); };
    ...
    this.$("#btnReset").onclick = () => this._resetUpload();
    this.$("#btnExtract").onclick = () => this._extractImage();
  }
  ...
  async _extractImage() {
    ...
    try {
      const result = await API.ocrExtract(this.imageFile, this.currentType);
      ...
      Toast.success(`Trích xuất thành công ${this.extractedItems.length} mục`);
      this._renderResults();
    } catch (err) {
  ...
  _renderResults() {
    ...
    if (this.currentType === "schedule") this._renderScheduleResults(area);
    else this._renderTranscriptResults(area);
  }

  // ── Kết quả: Thời khóa biểu ──────────────────────────────────────────────

  _renderScheduleResults(area) {
    area.innerHTML = `
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">3. Xác nhận và chỉnh sửa lịch học</h3>
          <span class="badge badge-success">${this.extractedItems.length} buổi</span>
        </div>
        ...
        <div class="table-wrap">
          <table class="table">
            <thead>
              <tr><th>Thứ</th><th>Giờ BĐ</th><th>Giờ KT</th>
                <th>Tên môn</th><th>Mã môn</th><th>Phòng</th><th>GV</th><th>Tuần</th><th></th></tr>
            </thead>
            <tbody></tbody>
          </table>
        </div>
    ...
    const renderRows = () => {
      tbody.innerHTML = "";
      this.extractedItems.forEach((item, idx) => {
        const row = document.createElement("tr");
        row.innerHTML = `
          <td>
            <select class="form-control" style="padding:4px;font-size:12px" data-field="day_of_week" data-idx="${idx}">
              ${[2,3,4,5,6,7,8].map(d => `<option value="${d}" ${item.day_of_week==d?"selected":""}>${Formatter.dayName(d)}</option>`).join("")}
            </select>
          </td>
          ...
          <td><input class="form-control" style="padding:4px;font-size:12px;min-width:160px" data-field="subject_name" data-idx="${idx}" value="${Formatter.escapeHtml(item.subject_name||"")}"></td>
          <td><input class="form-control" style="padding:4px;font-size:12px;width:80px" data-field="subject_code" data-idx="${idx}" value="${Formatter.escapeHtml(item.subject_code||"")}"></td>
          <td><input class="form-control" style="padding:4px;font-size:12px;width:90px" data-field="room" data-idx="${idx}" value="${Formatter.escapeHtml(item.room||"")}"></td>
          <td><input class="form-control" style="padding:4px;font-size:12px;width:120px" data-field="teacher" data-idx="${idx}" value="${Formatter.escapeHtml(item.teacher||"")}"></td>
          <td><input class="form-control" style="padding:4px;font-size:12px;width:80px" data-field="weeks" data-idx="${idx}" value="${Formatter.escapeHtml(item.weeks||"")}"></td>
          <td><button class="btn btn-ghost btn-sm" data-remove="${idx}">Xóa</button></td>
        `;
        tbody.appendChild(row);
      });
      tbody.querySelectorAll("[data-field]").forEach(el => {
        el.onchange = () => { this.extractedItems[parseInt(el.dataset.idx)][el.dataset.field] = el.value; };
      });
      tbody.querySelectorAll("[data-remove]").forEach(b => {
        b.onclick = () => { this.extractedItems.splice(parseInt(b.dataset.remove), 1); renderRows(); };
      });
    };
    renderRows();

    area.querySelector("#btnCancelSave").onclick = () => area.classList.add("hidden");
    area.querySelector("#btnSaveAll").onclick = () => this._saveSchedules(area);
  }

  async _saveSchedules(area) {
    if (this.extractedItems.length === 0) { Toast.warning("Không có dữ liệu để lưu"); return; }
    try {
      const payload = this.extractedItems.map(it => ({
  ...
  _renderTranscriptResults(area) {
    area.innerHTML = `
      <div class="card">
        <div class="card-header">
          <h3 class="card-title">3. Xác nhận và chỉnh sửa điểm</h3>
          <span class="badge badge-success">${this.extractedItems.length} môn</span>
        </div>
        <div class="alert alert-warning">
          Hãy kiểm tra dữ liệu trích xuất bên dưới. Bạn có thể chỉnh sửa, xóa hoặc thêm trước khi lưu.
        </div>
        <div class="table-wrap">
          <table class="table">
            <thead>
              <tr><th>Mã môn</th><th>Tên môn</th><th>TC</th><th>HK</th>
                <th>Điểm QT</th><th>Điểm thi</th><th>Tổng /10</th><th></th></tr>
            </thead>
            <tbody></tbody>
          </table>
        </div>
    ...
    const renderRows = () => {
      ...
      this.extractedItems.forEach((item, idx) => {
        const row = document.createElement("tr");
        row.innerHTML = `
          <td><input class="form-control" style="padding:4px;font-size:12px;width:90px" data-field="subject_code" data-idx="${idx}" value="${Formatter.escapeHtml(item.subject_code||"")}"></td>
          <td><input class="form-control" style="padding:4px;font-size:12px;min-width:200px" data-field="subject_name" data-idx="${idx}" value="${Formatter.escapeHtml(item.subject_name||"")}"></td>
          ...
          <td><input class="form-control" style="padding:4px;font-size:12px;width:80px" data-field="semester" data-idx="${idx}" value="${Formatter.escapeHtml(item.semester||"")}"></td>
      ...
      tbody.querySelectorAll("[data-field]").forEach(el => {
        el.onchange = () => {
          ...
          this.extractedItems[idx][el.dataset.field] = val;
        };
      });
      tbody.querySelectorAll("[data-remove]").forEach(b => {
        b.onclick = () => { this.extractedItems.splice(parseInt(b.dataset.remove), 1); renderRows(); };
    ...
    area.querySelector("#btnCancelSave").onclick = () => area.classList.add("hidden");
    area.querySelector("#btnSaveAll").onclick = () => this._saveSubjects(area);
  }

  async _saveSubjects(area) {
    if (this.extractedItems.length === 0) { Toast.warning("Không có dữ liệu để lưu"); return; }
    try {
      const payload = this.extractedItems.map(it => ({
        subject_code: it.subject_code || null,
