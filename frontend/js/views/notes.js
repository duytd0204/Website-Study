/**
 * NotesView - Quản lý ghi chú (Use Case 3.7-3.9).
 * Combobox lọc theo nhãn, nút 3 chấm mở context menu (Sửa/Ghim/Xóa).
 */
class NotesView extends BaseView {
  ...
  async render() {
    ...
    this.tags = await API.getNoteTags().catch(() => []);
    this._renderPage();
  }
  ...
  _renderPage() {
      ...
      <div class="card mb-4">
        <div style="display:flex;gap:12px;align-items:center;flex-wrap:wrap">
          <input type="text" id="noteSearch" placeholder="Tìm theo tiêu đề hoặc nội dung..."
            class="form-control" style="flex:1;min-width:240px" value="${Formatter.escapeHtml(this.searchText)}">
          <select id="tagSelect" class="form-control" style="width:200px">
            <option value="">Tất cả nhãn</option>
            ${(this.tags||[]).map(t => `<option value="${Formatter.escapeHtml(t)}" ${this.activeTag===t?"selected":""}>${Formatter.escapeHtml(t)}</option>`).join("")}
          </select>
        </div>
      </div>
    ...
    this._renderNotesGrid();
    this.$("#noteSearch").oninput = (e) => { this.searchText = e.target.value; this._renderNotesGrid(); };
    this.$("#tagSelect").onchange = (e) => { this.activeTag = e.target.value; this._renderNotesGrid(); };
    this.$("#btnAddNote").onclick = () => this._openNoteModal();
  }

  _renderNotesGrid() {
    const gridEl = this.$("#notesGrid");
    const filtered = this.notes.filter(n => {
      ...
      const matchTag = !this.activeTag || n.tag === this.activeTag;
    ...
    if (filtered.length === 0) {
      gridEl.innerHTML = `<div class="empty-state">
        ...
        <div class="empty-state-desc">${this.searchText || this.activeTag ? "Không tìm thấy ghi chú phù hợp" : "Nhấn '+ Tạo ghi chú' để bắt đầu"}</div>
      </div>`;
      return;
    }
    ...
    gridEl.innerHTML = `<div class="notes-grid">${filtered.map(n => `
      <div class="note-card ${n.is_pinned?'pinned':''}" style="background:${n.color}" data-id="${n.id}">
        <div class="note-title">${Formatter.escapeHtml(n.title)}</div>
        <div class="note-content">${Formatter.escapeHtml(n.content)}</div>
        <div class="note-meta">
          ${n.tag?`<span class="note-tag">${Formatter.escapeHtml(n.tag)}</span>`:'<span></span>'}
          <span>${Formatter.timeAgo(n.updated_at)}</span>
        </div>
    ...
    gridEl.querySelectorAll(".note-card").forEach(el => {
      ...
      el.addEventListener("click", (e) => {
        if (!e.target.classList.contains("note-menu-btn")) {
          this._editNote(parseInt(el.dataset.id));
        }
      });
    });
    ...
    gridEl.querySelectorAll(".note-menu-btn").forEach(btn => {
      btn.addEventListener("click", (e) => {
        e.stopPropagation();
        this._openContextMenu(e, parseInt(btn.dataset.id));
      });
    });
  }

  _openContextMenu(event, noteId) {
    ...
    menu.style.cssText = `
      ...
      border-radius:var(--radius); box-shadow:var(--shadow-lg); z-index:9000;
      min-width:160px; padding:4px 0; font-size:13px; animation:fadeIn 0.12s ease;`;

    const actions = [
      { label: "Chỉnh sửa", fn: () => this._editNote(noteId) },
      { label: note.is_pinned ? "Bỏ ghim" : "Ghim lên đầu", fn: () => this._togglePin(note) },
      { label: "Xóa ghi chú", fn: () => this._deleteNoteById(noteId), danger: true },
    ];

    menu.innerHTML = actions.map(a => `
      <div class="ctx-item" style="padding:8px 16px;cursor:pointer;${a.danger?'color:var(--danger)':'color:var(--text-primary)'}">
        ${a.label}
      </div>`).join("");
  ...
  async _deleteNoteById(id) {
    ...
    try {
      ...
      this.notes = await API.getNotes();
      this.tags = await API.getNoteTags();
      this._renderPage();
    } catch(err) { Toast.error(err.message); }
  }

  _editNote(id) {
    this._openNoteModal(this.notes.find(n => n.id === id));
  }

  _openNoteModal(item = null) {
    const isEdit = !!item;
    ...
    formEl.innerHTML = `
      <div class="form-group">
        ...
        <input name="title" class="form-control" value="${Formatter.escapeHtml(data.title)}" required placeholder="Tiêu đề ghi chú">
      </div>
      <div class="form-group">
        <label class="form-label">Nội dung <span class="required">*</span></label>
        <textarea name="content" class="form-control" rows="6" required placeholder="Nội dung ghi chú...">${Formatter.escapeHtml(data.content)}</textarea>
      </div>
      <div class="auth-form-row">
        <div class="form-group">
          <label class="form-label">Nhãn / Phân loại</label>
          <input name="tag" class="form-control" value="${Formatter.escapeHtml(data.tag||"")}" placeholder="Bài tập, Lý thuyết..." list="tag_suggestions">
          <datalist id="tag_suggestions">
            ${(this.tags||[]).map(t => `<option value="${Formatter.escapeHtml(t)}">`).join("")}
          </datalist>
        </div>
        <div class="form-group">
          <label class="form-label">Môn liên quan</label>
          <input name="related_subject" class="form-control" value="${Formatter.escapeHtml(data.related_subject||"")}" placeholder="CSE221">
        </div>
      </div>
      <div class="form-group">
    ...
    footer.querySelector('[data-action="save"]').onclick = () => this._saveNote(formEl, modal, item);
  ...
  async _saveNote(formEl, modal, item) {
    const isEdit = !!item;
...
BaseView.register("notes", NotesView);
