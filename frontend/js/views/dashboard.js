/**
 * DashboardView - Trang tổng quan.
 * Kế thừa BaseView (Inheritance), override render() (Polymorphism).
 */
class DashboardView extends BaseView {
  async render() {
    ...
    this.setHTML(this._buildHTML(gpa, schedules, upcoming, notes));
  }

  _buildHTML(gpa, schedules, upcoming, notes) {
    ...
    return `
      ...
      <div class="grid-2">
        <div class="card">
          ...
          <div id="upcomingClasses">${this._renderUpcoming(upcoming)}</div>
        </div>
        ...
        <div>
          ...
          <div class="card">
            ...
            ${this._renderRecentNotes(notes)}
          </div>
        </div>
      </div>

      ${this._renderSemesterTable(gpa.by_semester)}
    `;
  }

  _renderUpcoming(upcoming) {
    if (upcoming.length === 0) {
      return `<div class="empty-state"><p class="text-muted">Không có lịch học trong 24h tới</p></div>`;
    }
    return upcoming.slice(0, 5).map(c => `
      <div style="display:flex;gap:14px;padding:12px;border-radius:var(--radius);background:var(--bg-hover);margin-bottom:8px;align-items:center">
        <div style="width:50px;text-align:center;padding:8px;background:var(--tlu-primary);color:white;border-radius:var(--radius-sm)">
          <div style="font-size:11px;font-weight:600;opacity:0.85">${Formatter.dayName(new Date(c.next_class_at).getDay() === 0 ? 8 : new Date(c.next_class_at).getDay() + 1)}</div>
          <div style="font-size:16px;font-weight:800">${c.start_time}</div>
        </div>
        <div style="flex:1;min-width:0">
          <div style="font-weight:700;color:var(--text-primary)">${Formatter.escapeHtml(c.subject_name)}</div>
          <div class="text-sm text-muted">${c.room ? "Phòng " + Formatter.escapeHtml(c.room) : ""} ${c.teacher ? "· " + Formatter.escapeHtml(c.teacher) : ""}</div>
        </div>
  ...
  _renderRecentNotes(notes) {
    if (notes.length === 0) {
      return `<div class="empty-state"><p class="text-muted text-sm">Chưa có ghi chú nào</p></div>`;
    }
    return notes.slice(0, 3).map(n => `
      <div style="padding:10px;border-radius:var(--radius);background:${n.color || '#FFF9C4'};margin-bottom:8px;cursor:pointer" onclick="navigateTo('notes')">
        <div style="font-weight:700;font-size:13px;color:var(--text-primary)">${Formatter.escapeHtml(n.title)}</div>
        <div style="font-size:12px;color:var(--text-secondary);overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:2px">${Formatter.escapeHtml(n.content)}</div>
        <div class="text-sm text-muted" style="margin-top:4px;font-size:11px">${Formatter.timeAgo(n.updated_at)}</div>
      </div>
    `).join('');
  }

  _renderSemesterTable(bySemester) {
    if (!bySemester || bySemester.length === 0) return '';
    return `
      <div class="card mt-4">
        <div class="card-header"><h3 class="card-title">Tiến độ học tập theo học kỳ</h3></div>
        <div class="table-wrap">
          <table class="table">
            ...
            <tbody>
              ${bySemester.map(s => `
                <tr>
                  <td><b>${Formatter.escapeHtml(s.semester)}</b></td>
